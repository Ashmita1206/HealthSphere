...
describe('F35 — Docker Production Environment Suite', () => {
  ...
  it('7. Dockerignore files: correctly excludes node_modules, .env, and build artifacts', () => {
    const rootIgnore = path.join(rootDir, '.dockerignore');
    const serverIgnore = path.join(rootDir, 'server/.dockerignore');

    expect(fs.existsSync(rootIgnore)).toBe(true);
    expect(fs.existsSync(serverIgnore)).toBe(true);

    const rootContent = fs.readFileSync(rootIgnore, 'utf-8');
    expect(rootContent).toMatch(/node_modules/i);
    expect(rootContent).toMatch(/\.env/i);

    const serverContent = fs.readFileSync(serverIgnore, 'utf-8');
    expect(serverContent).toMatch(/node_modules/i);
    expect(serverContent).toMatch(/\.env/i);
  });

  it('8. Development and Production env profiles are standardized', () => {
    const prodEnv = path.join(rootDir, '.env.production.example');
    const devEnv = path.join(rootDir, '.env.development.example');

    expect(fs.existsSync(prodEnv)).toBe(true);
    expect(fs.existsSync(devEnv)).toBe(true);

    const prodContent = fs.readFileSync(prodEnv, 'utf-8');
    expect(prodContent).toMatch(/NODE_ENV=production/i);
    expect(prodContent).toMatch(/JWT_SECRET=/i);

    const devContent = fs.readFileSync(devEnv, 'utf-8');
    expect(devContent).toMatch(/NODE_ENV=development/i);
    expect(devContent).toMatch(/JWT_SECRET=/i);
  });
});
