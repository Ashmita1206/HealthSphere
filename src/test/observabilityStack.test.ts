...
describe('F38 — Monitoring & Observability Stack Suite', () => {
  ...
  describe('4. Observability Endpoint Controllers', () => {
    ...
    it('getHealth includes CPU monitoring cores and load average', async () => {
      let body: { cpu?: { cores: number; loadAvg: number[] } } | null = null;
      const mockRes = {
        status: () => ({
          json: (data: typeof body) => {
            body = data;
  ...
  // ----------------------------------------------------
  // 5. Morgan, Error Tracking & Log Rotation
  // ----------------------------------------------------
  describe('5. Logging & Error Tracking Extensions', () => {
    it('provides morgan HTTP logging middleware and central error tracking', () => {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const logger = require('../../server/utils/logger');
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { morganMiddleware } = require('../../server/utils/logger');

      expect(typeof morganMiddleware).toBe('function');
      expect(typeof logger.trackError).toBe('function');

      const tracked = logger.trackError(new Error('Sample test error'), { subsystem: 'telemedicine' });
      expect(tracked.name).toBe('Error');
      expect(tracked.message).toBe('Sample test error');
      expect(tracked.context.subsystem).toBe('telemedicine');
    });
  });
});
