...
describe('F37 — Enterprise Security Hardening Suite', () => {
  ...
  // ----------------------------------------------------
  it('7. CSRF Protection: generates secure token and verifies matching pairs', () => {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { generateCsrfToken, verifyCsrfToken } = require('../../server/middlewares/security');
    ...
    expect(verifyCsrfToken(token, 'different-fake-token')).toBe(false);
